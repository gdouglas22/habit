        ...
        {products.length === 0 && (
          ...
          >
            {state.products.length === 0
              ? "Пока пусто. Добавь продукт или целое блюдо — вручную или через ИИ."
              : "Ничего не найдено"}
          </div>
        )}
          ...
          {products.map((p) => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 800, fontSize: 16, color: "var(--text)" }}>{p.name}</div>
                <div style={{ fontWeight: 700, fontSize: 13, color: "var(--hint)", marginTop: 2 }}>
                  {Math.round(p.kcal)} ккал · Б {Math.round(p.protein)} Ж {Math.round(p.fat)} У{" "}
                  {Math.round(p.carbs)} / 100 г
                </div>
              </div>
              <div
                onClick={(e) => {
